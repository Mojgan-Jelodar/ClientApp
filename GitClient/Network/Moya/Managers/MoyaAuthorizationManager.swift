...
struct MoyaAuthorizationManager : AuthorizationManager {
    func accessToken(with: AccessToken) -> AnyPublisher<AccessTokenResponse, Error> {
        MoyaCombineNetworkExecuter.shared.makeNetworkRequestForAPIService(service:
                                                                            MultiTarget(AuthorizationApi.accessToken(params: with.wsModel())),networkProvider: sharedNetworkProvider)
