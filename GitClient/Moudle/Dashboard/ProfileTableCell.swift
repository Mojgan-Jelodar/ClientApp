...
final class ProfileTableCell: UITableViewCell {
    ...
    private lazy var followersLbl : UILabel = {
        ...
        tmp.font = UIFont.boldSystemFont(ofSize: LayoutConstant.Common.fontSize)
        tmp.lineBreakMode = .byTruncatingTail
        return tmp
    }()
    ...
    func setupView() {
        func setAvatarConstraint() {
            avatarImgView.snp.makeConstraints { maker in
                maker.leading.equalTo(LayoutConstant.Common.margin)
                maker.centerY.equalToSuperview()
                maker.size.equalTo(LayoutConstant.AvatarImage.size)
            }
        }
        func setNameConstraint() {
            nameLbl.snp.makeConstraints { maker in
                maker.leading.equalTo(avatarImgView.snp.trailing).offset(LayoutConstant.Common.margin)
                maker.centerY.equalTo(avatarImgView).offset(LayoutConstant.Common.margin * -1)
            }
        }
        ...
        func setStackConstraint() {
            stackView.snp.makeConstraints { maker in
                maker.leading.equalTo(avatarImgView.snp.trailing)
                maker.trailing.equalToSuperview()
                maker.top.equalTo(emailLbl.snp.bottom).offset(LayoutConstant.Common.margin)
